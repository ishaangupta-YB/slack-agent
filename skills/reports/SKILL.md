...
- `weekly_report` — generates the Moon Bot weekly ops report. It queries Elasticsearch for 7-day log volume, error rate, rate-limiting mentions, and Gitaly-related logs. If Elasticsearch is not configured, it returns a helpful fallback template.
...
You can also use the `/moonbot report weekly` and `/moonbot report deploy <ts>` slash commands for the same output.
