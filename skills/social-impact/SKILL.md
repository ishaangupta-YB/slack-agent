...
Ishu is designed to help *any* team operate like a well-staffed platform team — including nonprofits, civic tech groups, open-source maintainers, and public-health organizations that do not have dedicated SREs, data engineers, or 24/7 on-call rotations.

Use Ishu to turn a Slack thread into an incident-response, sustainability, or accessibility workspace so under-resourced teams can keep critical services online.

## Why this matters

- **Nonprofit operations** often rely on a handful of volunteers wearing multiple hats. Ishu collapses log analysis, GitHub triage, Slack history search, and status monitoring into one conversation.
...
- **Open-source sustainability** depends on maintainers who may be spread across time zones. Ishu can search code, file issues, open PRs, and generate weekly health reports so maintainers spend less time context-switching.
...
Ishu should call `public_status` with the JSON endpoint, then summarize the indicator and description.

When `STATUS_MONITOR_CHANNEL` and `STATUS_MONITOR_PAGES` are configured, Ishu also polls those pages on a cron schedule, posts a Slack alert the first time an incident is detected, and posts a recovery notice when the service returns to an operational indicator.
...
Ishu can clone or search the repo, open/view issues, and summarize recent GitHub activity.

### Weekly status for a nonprofit tech team

Use the scheduled weekly report (or `/ishu report weekly`) to summarize:
...
After resolving an incident, Ishu can:
...
- Guest users and unauthenticated channels should not have access to incident data. Ishu enforces tiered access and guest refusal by default.
