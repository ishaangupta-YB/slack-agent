...
| **App Home + slash command** | Home tab overview and `/moonbot help | demo | status | diagnose | ping | whoami | search | report | statuspage` for quick discovery. |
...
9. **Live diagnostics, demo prompts, LLM ping, and identity:**  
   `/moonbot diagnose`, `/moonbot demo`, `/moonbot ping`, and `/moonbot whoami`
...
| 2:45–2:55 | Security + trace viewer | Show `/moonbot diagnose`, `/moonbot ping`, `/moonbot status`, `/moonbot whoami`, `/moonbot demo`, tiered access explanation, and the HTML trace viewer stepping through a session. |
