...
function writeStore(store: MemoryStore) {
  ensureStorage();
  writeFileSync(cfg.agent.memoryFile, JSON.stringify(store, null, 2));
}

export function getMemoryRecent(limit: number): MemoryEntry[] {
...
export const memoryTool: Tool = {
  ...
  run(input) {
    ...
    if (input.mode === "search") {
      ...
      entries = searchMemory(input.query, limit);
    } else {
      entries = getMemoryRecent(limit);
    }
